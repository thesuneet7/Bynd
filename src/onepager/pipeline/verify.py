...
from ..schemas import Claim, Entailment, FinancialCell, Verification
...
def verify_claims(ctx: RunContext, claims: list[Claim]) -> None:
    ...
    ctx.note(f"[verify] checking {len(reported)} claims with Grok...")
    for i in range(0, len(reported), _BATCH):
        batch = reported[i : i + _BATCH]
    ...
    # Derived cells inherit entailment from their base cells.
    by_id = {c.id: c for c in reported}
    for d in derived:
